...
Сам ansible playbook и inventory file для ansible лежит в репозитории git@github.com:soldatviktimnosti/ansible.git

Также в этом репозитории есть два скрипта:

**git-monitor.sh** и **deploy-infra.sh**

**git-monitor.sh** - запускается у меня на хосте и смотрит есть ли новые коммиты в этот репозиторий. 
...
Теперь можно будет подключаться к ВМ без указания их директории - а она при каждой итерации скрипта новая, и это неудобно.
